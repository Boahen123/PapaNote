...
class App(customtkinter.CTk):
    ...
    def __init__(self):
        ...
        self.frame_button = tk.Frame(self, relief=tk.SUNKEN, width=100, height=100, bd=3, background='grey')
        self.frame_button.grid(row=0, column=0, sticky="ns")
        
        # The open button
        self.button_open = customtkinter.CTkButton(master=self.frame_button, text='OPEN FILE', command=self.open_file)  
        self.button_open.grid(row=0, column=0, padx=5, pady=5)
        
        # The save button
        self.button_save = customtkinter.CTkButton(master=self.frame_button, text='SAVE AS', command=self.save_file)
